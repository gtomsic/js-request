...
// CODE HERE
const changeSayHelloBackground = () => {
  sayHelloButton.classList.remove('defaultBg');
  sayHelloButton.classList.add('changeBg');
};
sayHelloButton.addEventListener('mouseover', changeSayHelloBackground);
...
sayHelloButton.addEventListener('mouseleave', () => {
  sayHelloButton.classList.remove('changeBg');
  sayHelloButton.classList.add('defaultBg');
});
...
const repeatMyParam = () => {
  ...
  axios.get(`http://localhost:3000/repeat/gabriel`).then((res) => {
    console.log(res.data);
...
// CODE HERE
// const queryFunction = () => {
//   axios.get(`http://localhost:3000/query-test?name="Gabriel"`).then((res) => {
//     console.log(res.data);
//   });
// };
...
//   .addEventListener('click', queryFunction);
...
// document.querySelector('#query-button').addEventListener('click', queryNoParam);

const queryFunction = () => {
  axios
    .get(`http://localhost:3000/query-test?name="Gabriel"&last="Tomsic"`)
    .then((res) => {
      console.log(res.data);
    });
};
...
// CODE HERE
const formAddFood = document.querySelector('form');
const inputFood = document.querySelector('#add-food');
const addFoodFunction = (e) => {
  e.preventDefault();
  const body = { newFood: inputFood.value };
...
formAddFood.addEventListener('submit', addFoodFunction);
